...
KE.box = function(cf){
    ...
    var t = KE.util.getTop(KE.g[cf.id].containerDiv) +
    Math.round(parseInt(KE.g[cf.id].height) / 2) - Math.round(cf.height / 2);
    var l = KE.util.getLeft(KE.g[cf.id].containerDiv) +
    Math.round(parseInt(KE.g[cf.id].width) / 2) - Math.round(cf.width / 2);
    if (Math.round(parseInt(KE.g[cf.id].height) / 2) < Math.round(cf.height / 2))
        t = KE.util.getTop(KE.g[cf.id].containerDiv);
    if (Math.round(parseInt(KE.g[cf.id].width) / 2) < Math.round(cf.width / 2))
        l = KE.util.getLeft(KE.g[cf.id].containerDiv);
    div.style.top = t + 'px';
    div.style.left = l + 'px';
    this.div = div;
    ...
    this.getDoc = function()
    {
        ...
        html += '<meta http-equiv="Content-Type" content="text/html; charset=utf-8">';
        html += '<link href="' + KE.scriptPath + 'skins/' + KE.g[this.cf.id].skinType +
...
KE.plugin['link'] = {
    ...
    click : function(id)
    {
        ...
        'hyperLink" value="http://" style="width:190px;" class="ke-text-input"></td></tr>';
        html += '</table>';
        var box = new KE.box({
                id : id,
                cmd : cmd,
...
KE.plugin['media'] = {
    ...
    click : function(id)
    {
        ...
        html += '<tr><td colspan="2"><div border="0"><tr><td id="'+
        cmd+'preview" style="width:240px;height:200px;border:1px solid #AAAAAA;background-color:#FFFFFF;" align="center" valign="middle">&nbsp;</td></tr></table></td></tr>';
        html += '<tr><td style="width:40px;padding:5px;">远程</td>';
        html += '<td style="width:210px;padding-bottom:5px;"><input type="text" id="'+
        cmd+'link" value="http://" style="width:190px;border:1px solid #555555;" /></td></tr>';
        html += '</table>';
        var box = new KE.box({
                id : id,
                cmd : cmd,
